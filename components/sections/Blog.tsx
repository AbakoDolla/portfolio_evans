      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <motion.div className="text-center mb-12"
          ...
          <div className="inline-flex items-center gap-3 mb-4">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold">
              Tech <span className="text-gradient">en direct</span>
            </h2>
          </div>
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button key={cat} onClick={() => setFilter(cat)}
                ...
                {Icon && <Icon className="w-3.5 h-3.5" />} {cat === "Tous" ? "Toutes catégories" : cat}
              </button>
            );
          })}
        ...
        {loading ? (
          <div className="flex flex-col items-center py-20 gap-3">
            ...
            <p className="text-muted-foreground font-mono text-sm">Récupération des actus…</p>
          </div>
        ) : error ? (
          <div className="text-center py-16">
            <p className="text-muted-foreground mb-4">Impossible de charger les actus pour le moment.</p>
            <button onClick={load} className="px-4 py-2 glass rounded-xl text-sm text-primary hover:bg-primary/10 transition-all">Réessayer</button>
          </div>
        ) : (
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="text-center py-16 text-muted-foreground font-mono">Aucun article dans cette catégorie.</div>
        )}
      </div>
    </section>
  );
